...
export function DashboardTopbar({ leftContent, userRole = 'student' }) {
  return (
    <header className="flex h-20 shrink-0 items-center justify-between border-b border-[#e2e8d5] bg-[#f6f9f3] px-8">
      <div className="flex flex-1 items-center gap-8">
        {userRole === 'admin' && leftContent ? (
          leftContent
        ) : (
          <div className="relative w-full max-w-lg">
            ...
            <input
              type="text"
              placeholder="Explore contests, skills, or mentors..."
      ...
      <div className="flex shrink-0 items-center justify-end gap-6 ml-4">
        ...
        <div className="flex cursor-pointer items-center gap-3 border-l border-gray-200 pl-6 group">
          ...
          <div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full border-2 border-lime-300 bg-lime-100">
            <img
              src={`https://ui-avatars.com/api/?name=${userRole === 'admin' ? 'Admin' : 'Alex+Rivera'}&background=446611&color=fff`}
              alt="Avatar"
