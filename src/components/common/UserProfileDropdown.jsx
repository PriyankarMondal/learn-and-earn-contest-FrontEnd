...
export function UserProfileDropdown({ theme = 'light' }) {
  ...
  const avatarUrl = user.avatar || `https://ui-avatars.com/api/?name=${user.name.split(' ').join('+')}&background=446611&color=fff`
    ...
    <div className="relative">
      ...
      >
        ...
        <div className="hidden sm:block text-left shrink-0">
          <div className={`text-[10px] font-black leading-none truncate max-w-[100px] 
            ${isDark ? 'text-white' : 'text-gray-900'}`}>{user.name}</div>
          <div className={`text-[8px] font-bold uppercase tracking-tighter mt-0.5 
            ${isDark ? 'text-lime-400' : 'text-lime-600'}`}>{user.role}</div>
        </div>
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute right-0 mt-3 w-56 rounded-2xl bg-white border border-gray-100 shadow-2xl z-50 overflow-hidden py-2 animate-in slide-in-from-top-2 duration-200">
