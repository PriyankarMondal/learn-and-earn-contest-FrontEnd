...
export function ActiveContestsList() {
  ...
  const handleAction = (contest) => {
    ...
    setModalView(contest.button === 'SUBMIT WORK' ? 'participate' : 'details')
  }
    ...
    <div className="mb-10 text-left">
      ...
      <div className="flex flex-col gap-4">
        {activeContestsData.map((contest) => (
          <div key={contest.id} className="flex flex-col sm:flex-row gap-5 rounded-xl bg-white p-4 sm:pr-6 shadow-sm border border-gray-100 items-start sm:items-center overflow-hidden transition-all hover:shadow-md">
            <div className={`h-24 w-full sm:w-24 shrink-0 rounded-lg ${contest.image} relative overflow-hidden flex items-center justify-center`}>
            ...
            <div className="flex-1 min-w-0 w-full">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-2 gap-2">
                <h3 className="text-base font-black text-gray-900 truncate">{contest.title}</h3>
                <div className="flex gap-2">
                  {contest.tags.map(tag => (
                    <span key={tag.label} className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider whitespace-nowrap ${tag.bg}`}>
                      {tag.label}
                    </span>
                  ))}
                </div>
              ...
              <p className="text-xs font-medium text-gray-500 mb-4 max-w-lg leading-relaxed">
                {contest.desc}
              </p>
            ...
            <div className="w-full sm:w-auto mt-4 sm:mt-0 sm:pl-4 sm:border-l border-gray-100">
              <button
                ...
                className={`${contest.btnVariant} w-full sm:w-auto px-5 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all shadow-sm active:scale-95`}
              >
                {contest.button}
              </button>
            </div>
